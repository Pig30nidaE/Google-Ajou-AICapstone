...
| 논문 분석·데이터 대조 | ✅ 완료 |
| 사전 문서 6종 | ✅ 완료 |
| unit test | ✅ **105개 전부 통과** |
| `20260803_013228_full` 감사 | ❌ 재현 실패 (raw VAE 붕괴, C의 SMOTE no-op) |
| `20260803_041244_full` 감사 | ⚠️ **부분 재현** — A5는 논문 수준 근접, 그러나 결함 3건 잔존 |
| 3건 교정 (R-1·R-2·R-3) | 🔧 코드 수정 완료 — **재실행 필요** |
| 교정본 학습·평가 | ⏳ **미실행** |

---

## 2026-08-03 두 번째 산출물 감사 (`20260803_041244_full`)

### 판정: 부분 재현

**회복된 것 ✅**

| 항목 | 1차 (`013228`) | 2차 (`041244`) |
| --- | ---: | ---: |
| VAE 재구성 MSE | 43,447,208 | **0.261** (scaled) |
| 합성 셀 범위 이탈률 | 71.36% | **0.42%** |
| C의 SMOTE `n_synthetic` | 0 (no-op) | 정상 생성 |
| A, Wide&Deep+VAE macro-F1 | 0.4524 | **0.8317** (논문 0.8556) |
| A, DNN 증강 없음 macro-F1 | — | **0.8636** |

A5 기록 단위 성능이 **논문 수준에 근접**했다. Wide&Deep은 논문과 0.024 차,
DNN은 논문의 다른 모델 수치대와 비슷한 범위다.

**남은 결함 3건 🔴 (이번에 교정)**

| # | 결함 | 증거 |
| --- | --- | --- |
| R-1 | **합성자료 분산 붕괴** — 46/46 변수의 표준편차가 실제의 30% | 평균 이동은 0.19σ로 정상인데 σ비 중앙값 0.301. TRTS `mean_target_proba` 0.07 |
| R-2 | **TabNet이 다수 클래스 해에서 조기 종료** | B에서 balanced acc 정확히 0.3333(단일 클래스 예측)이 6개 중 4개. A5 macro-F1 0.5264 대 DNN 0.8636 |
| R-3 | **inner 탐색 예산이 VAE에 편중** | 예산 24에서 vae 12 대 none 4 — "여러 번 시도해 이기는" 선택 편향 |

R-1이 가장 중대하다. **RQ3("VAE 증강이 새 피험자 일반화를 향상시키는가")의 답이
모델링 결함과 구분되지 않았다** — 실험 B에서 VAE 증강의 Dem recall이 0에 머문 것이
"증강이 무효"인지 "붕괴한 증강이 무효"인지 판정할 수 없었다.

원인·교정 내용은 [report_inconsistencies.md](report_inconsistencies.md) 부록 R-1~R-3 참조.

### 누수 통제의 효과 (연구 결과로서는 유효)

| 검증방식 | Macro-F1 | Balanced Acc | Macro ROC-AUC | Dem Recall |
| --- | ---: | ---: | ---: | ---: |
| A5 (행 단위 분할·전처리 누수) | 0.9756 | 0.9708 | 0.9987 | 12/12 |
| B (피험자 독립) | 0.3705 | 0.3809 | 0.5513 | 2/12 |
| C (Nested Group CV) | 0.3662 | 0.4630 | 0.5687 | 8/12 |

**피험자 단위로 분리하면 macro-F1이 0.98 → 0.37로 무너진다.** ROC-AUC 0.55는 거의 우연
수준이다. 이 방향성은 R-1 교정과 무관하게 유지될 가능성이 높지만(누수 제거의 효과가
증강 품질보다 훨씬 크다), **정확한 크기는 재실행 후에만 인용할 수 있다.**
