...
## 3. 과거 dry-run 실측 결과 (2026-08-02, 수정 전 참고값)

`python run.py --config <각 config> --dry-run`

2026-08-03 결과 감사 뒤 실험 A의 validation 적용과 경고 집계를 수정했다. 아래는
당시 코드의 기록이며 현재 코드의 검증 완료 표로 사용하지 않는다. 사용자가 요청한
대로 이번 수정에서는 실행하지 않았으므로 새 수치는 다음 dry-run에서 갱신한다.
