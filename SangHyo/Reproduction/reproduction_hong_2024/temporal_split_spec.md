...
피험자별로:
  ...
  3. test  = date >= cut
  4. train 후보 = date < cut
  5. validation = train 후보의 마지막 14일  # 논문 미보고 가정 A-06
  6. train = 그 이전
전체 피험자에 대해 concat
  ↓
...
논문은 early stopping을 사용했다고 쓰지만 validation 출처를 밝히지 않았다. 현재
실험 A는 test를 monitor로 쓰지 않으면서 이를 실제 적용하기 위해 train 쪽 마지막
14일을 분리한다. 따라서 14일은 논문 확정 방법이 아니라 결과에 반드시 남는 가정이다.
...
실험 A도 같은 validation 규칙을 쓴다. B1의 추가 통제는 `L-1`일 embargo이며,
sampling 전략도 피험자 균형 방식으로 바뀐다.
...
test 시퀀스 수는 변하지 않는다(590 / 426 / 294). validation과 embargo는 train
후보에서만 떼어 간다. 2026-08-03 감사 뒤 실험 A에도 validation을 실제 적용하도록
수정했으므로, train 시퀀스 수와 embargo 비용은 다음 dry-run에서 다시 산출해야 한다.

## 4. 분할 결과 (실측, `final_week_mode: calendar_days`)

| 항목 | 값 |
| --- | --- |
| validation 분리 전 train 후보 행 | 11,128 |
| test 행 | 1,043 |
| train 후보/test 피험자 | 174 / 174 (설계상 동일) |
