...
def run_pipeline(*, namespace=None, data_root=None, output_dir=None, mode=None,
    ...
    _write_status(output, {"status": "starting", "experiment": EXPERIMENT_NAME,
                           ...
                           "output_dir": str(output), "compute_note": "Google YDF (Yggdrasil) ensemble; CPU-friendly."})

    from SangHyo.Binary.Binary_Google_YDF_Ensemble.train import RunConfig, run_experiment

    if run_mode == "smoke":
        settings = dict(repeats=1, outer_folds=2, inner_folds=2)
    else:
        settings = dict(repeats=5, outer_folds=5, inner_folds=3)
