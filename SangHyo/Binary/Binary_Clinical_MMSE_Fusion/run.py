"""Single base.ipynb entry point for the wearable + MMSE fusion experiment.
...
Default mode is ``full``.  This experiment is a CPU-only robust tabular
ensemble; no GPU is required.  ``smoke`` is a fast wiring check only.
"""
...
EXPERIMENT_NAME = "Binary_Clinical_MMSE_Fusion"
...
DEFAULT_COLAB_RESULTS_ROOT = Path(
    "/content/drive/MyDrive/Binary_Clinical_MMSE_Fusion_result"
)
HARD_RUNTIME_SECONDS = 21_600   # 6h hard limit (this model finishes in minutes)
...
def _resolve_data_root(namespace: Mapping[str, Any], explicit: str | None) -> Path:
    ...
    for value in (explicit, namespace.get("DATA_ROOT"), os.environ.get("MMSE_DATA_ROOT")):
        if value:
...
def _resolve_output_dir(explicit: str | None) -> Path:
    ...
    override = os.environ.get("MMSE_OUTPUT_DIR")
    if override:
    ...
    *,
    ...
    _write_status(
        output,
        {
            ...
            "compute_note": "CPU-only robust tabular ensemble; no GPU required.",
        },
    )

    from SangHyo.Binary.Binary_Clinical_MMSE_Fusion.train import RunConfig, run_experiment

    if run_mode == "smoke":
        settings = dict(repeats=1, outer_folds=2, inner_folds=2, max_features=10)
    else:
        settings = dict(repeats=5, outer_folds=5, inner_folds=3, max_features=40)
