"""base.ipynb entry point — leakage-free LightGBM paper reproduction (no MMSE).

    USER_FOLDER = "SangHyo"
    RUN_FILE = "Binary_PaperLGBM_NoMMSE/run.py"

CPU-only. Reproduces the paper's rich lifelog features + LightGBM but evaluates
leakage-free (subject-level), and reports a leakage diagnostic that shows the
paper-style day-level split reaching ~0.9 via subject leakage.
"""
...
EXPERIMENT_NAME = "Binary_PaperLGBM_NoMMSE"
INCLUDE_MMSE = False
...
def _ensure_dependencies(skip_install: bool) -> None:
    required = {"numpy": "numpy", "pandas": "pandas", "scipy": "scipy",
                ...
                "joblib": "joblib", "lightgbm": "lightgbm"}
...
def run_pipeline(*, namespace=None, data_root=None, output_dir=None, mode=None,
    ...
    _write_status(output, {"status": "starting", "experiment": EXPERIMENT_NAME,
                           "mode": run_mode, "include_mmse": INCLUDE_MMSE,
                           "data_root": str(resolved_data), "output_dir": str(output)})

    from SangHyo.Binary.Binary_PaperLGBM_NoMMSE.paperlib import RunConfig, run_experiment

    settings = dict(repeats=1, folds=3) if run_mode == "smoke" else dict(repeats=5, folds=5)
    config = RunConfig(
        training_root=str(resolved_data / "1.Training"),
        validation_root=str(resolved_data / "2.Validation"),
        output_dir=str(output), include_mmse=INCLUDE_MMSE,
        experiment_name=EXPERIMENT_NAME, run_mode=run_mode,
