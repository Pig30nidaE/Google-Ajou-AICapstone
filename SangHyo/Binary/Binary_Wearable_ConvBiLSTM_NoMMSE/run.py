"""Single base.ipynb entry point for the wearable-only Conv1D+BiLSTM experiment.

base.ipynb usage (Cell 2)::

    USER_FOLDER = "SangHyo"
    RUN_FILE = "Binary_Wearable_ConvBiLSTM_NoMMSE/run.py"

Default mode is ``full``.  ``full`` uses a Colab GPU (A100 recommended) for the
Conv1D+BiLSTM; the tabular ensemble runs on CPU.  ``smoke`` is a fast CPU wiring
check only and is never reported as performance.
"""
...
EXPERIMENT_NAME = "Binary_Wearable_ConvBiLSTM_NoMMSE"
...
DEFAULT_COLAB_RESULTS_ROOT = Path(
    "/content/drive/MyDrive/Binary_Wearable_ConvBiLSTM_NoMMSE_result"
)
SOFT_RUNTIME_SECONDS = 20_400   # 5h40m soft budget
HARD_RUNTIME_SECONDS = 21_600   # 6h hard limit
...
def _alarm_handler(signum, frame):  # pragma: no cover - Colab/Linux path
    del signum, frame
...
def _resolve_data_root(namespace: Mapping[str, Any], explicit: str | None) -> Path:
    ...
    for value in (explicit, namespace.get("DATA_ROOT"), os.environ.get("WEARABLE_DATA_ROOT")):
        if value:
...
def _ensure_dependencies(skip_install: bool) -> None:
    ...
    missing = [
        dist for dist, module in required.items() if importlib.util.find_spec(module) is None
    ]
...
def _resolve_output_dir(explicit: str | None) -> Path:
    ...
    override = os.environ.get("WEARABLE_OUTPUT_DIR")
    if override:
        return Path(override).expanduser().resolve()
    if not DEFAULT_COLAB_RESULTS_ROOT.parent.is_dir():
        raise FileNotFoundError(
            "Google Drive not mounted at /content/drive/MyDrive. Run base.ipynb "
            "Cell 1 first, or pass --output-dir for a local smoke test."
        )
...
def run_pipeline(
    *,
    ...
    try:
        ...
        cuda = False
    if run_mode == "full" and not cuda:
        print(
            "[warn] CUDA not available; the Conv1D+BiLSTM will be skipped and only "
            "the CPU tabular ensemble will run. Select an A100 runtime for the full "
            "recipe."
        )
    ...
    from SangHyo.Binary.Binary_Wearable_ConvBiLSTM_NoMMSE.train import RunConfig, run_experiment

    if run_mode == "smoke":
        settings = dict(repeats=1, outer_folds=2, inner_folds=2, max_features=8, epochs=2)
    else:
        settings = dict(repeats=2, outer_folds=5, inner_folds=3, max_features=30, epochs=60)
