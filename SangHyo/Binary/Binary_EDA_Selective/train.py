"""End-to-end MMSE-domain fusion experiment: EDA, nested CV, freeze, evaluation."""
...
EXPERIMENT_NAME = "Binary_EDA_Selective"
...
def _eda(data: SubjectData) -> dict:
    ...
    return {
        ...
        "top_feature_direction_free_auc": aucs[:20],
    }


def _factories(data: SubjectData, config: RunConfig):
    return [
        ("logreg", lambda: TabularLearner(data, "logreg")),
        ("gbt_shallow", lambda: TabularLearner(data, "gbt_shallow")),
    ]


def _final_weights(model_mean_weight: dict[str, float], model_balacc: dict[str, float]) -> dict[str, float]:
...
def run_experiment(config: RunConfig, *, feature_subset=MINIMAL_FEATURES) -> dict:
    output = Path(config.output_dir)
    output.mkdir(parents=True, exist_ok=True)
    started = datetime.now(timezone.utc)

    train = load_split(config.training_root, require_labels=True, split="train", feature_subset=feature_subset)
    factories = _factories(train, config)
    ...
    nested_report = {
        ...
        "weight_gate": config.weight_gate, "models": [n for n, _ in factories],
        "cognitive_test_used": True,
    ...
    if config.evaluate_validation:
        validation = _freeze_and_evaluate(
            config, train, final_weights, thr_balanced, thr_accuracy, output, feature_subset
        )
...
def _freeze_and_evaluate(config, train, final_weights, thr_balanced, thr_accuracy, output, feature_subset):
    validation = load_split(config.validation_root, require_labels=False, split="val", feature_subset=feature_subset)
    ...
    for name in final_weights:
        learner = TabularLearner(train, name).fit(all_idx)
