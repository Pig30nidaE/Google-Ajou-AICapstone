"""Single base.ipynb entry point for the MMSE-domain fusion experiment.
...
CPU-only robust tabular ensemble; no GPU required. ``full`` is the default.
"""
...
def run_pipeline(*, namespace=None, data_root=None, output_dir=None, mode=None,
    ...
    _write_status(output, {"status": "starting", "experiment": EXPERIMENT_NAME,
                           ...
                           "output_dir": str(output), "compute_note": "CPU-only tabular ensemble."})

    from SangHyo.Binary.Binary_MMSE_DomainFusion.train import RunConfig, run_experiment

    if run_mode == "smoke":
        settings = dict(repeats=1, outer_folds=2, inner_folds=2, max_features=8)
    else:
        settings = dict(repeats=5, outer_folds=5, inner_folds=3, max_features=20)
