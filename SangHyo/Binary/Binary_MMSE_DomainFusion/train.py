...
EXPERIMENT_NAME = "Binary_MMSE_DomainFusion"
...
def run_experiment(config: RunConfig, *, feature_subset=None) -> dict:
...
def _freeze_and_evaluate(config, train, final_weights, thr_balanced, thr_accuracy, output, feature_subset):
    ...
    for name in final_weights:
        learner = TabularLearner(train, name, config.max_features).fit(all_idx)
