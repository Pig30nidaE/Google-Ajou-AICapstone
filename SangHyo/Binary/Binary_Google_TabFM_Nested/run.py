"""Single entry point for Binary_Google_TabFM_Nested.
...
Runtime: **GPU (T4) recommended** -- TabFM is a transformer doing in-context
inference, so a GPU shortens the run considerably; CPU works but is slower.
Right after loading the checkpoint the script prints a measured per-fit time
and a projected total, so an infeasible runtime is visible within the first
minute instead of an hour in.

Recommended order: ``--profile quick`` first (2 outer repeats, feasibility +
projection), then ``default`` (10 repeats, the primary protocol).

Pipeline stages (in order):
    1. environment check: install pinned Google TabFM (tag v1.0.1), fail-closed
    2. checkpoint load (once) + API probe on synthetic data + micro-benchmark
    3. label + feature build (subject-local, leakage-audited)
...
for _name in [m for m in list(sys.modules)
              if m == "tabfmnested" or m.startswith("tabfmnested.")]:
    sys.modules.pop(_name, None)


# ----------------------------------------------------------- environment ----
def _in_notebook_host() -> bool:
    """True when executed from inside a notebook kernel (see the CircadianNested
    post-mortem: runpy rewrites argv[0] but keeps the kernel's ``-f ...`` args)."""
...
def _parse_args() -> argparse.Namespace:
    ...
    if not argv and os.environ.get("BGTF_ARGS"):
        argv = shlex.split(os.environ["BGTF_ARGS"])
    return parser.parse_args(argv)


def _ensure_tabfm(stub_requested: bool, profile_name: str) -> None:
    """Install the pinned Google TabFM package, fail-closed (no substitution).

    The wiring stub is the single sanctioned exception: BGTF_WIRING_STUB=1 with
    profile=smoke skips the install entirely so machines that cannot build
    TabFM can still exercise the plumbing.  Any other profile with the stub is
    an error -- a stub result must never look like a Google-TabFM result.
    """

    if stub_requested:
        if profile_name != "smoke":
            raise RuntimeError(
                "BGTF_WIRING_STUB=1 is only allowed with --profile smoke; "
                "formal profiles require the real Google TabFM."
            )
        return
    ...
    from tabfmnested.config import TABFM_PIP_SPEC

    if importlib.util.find_spec("tabfm") is None:
        print(f"[env] tabfm not found; installing '{TABFM_PIP_SPEC}' ...", flush=True)
        result = subprocess.run(
            [sys.executable, "-m", "pip", "install", "--quiet", TABFM_PIP_SPEC],
            check=False,
        )
        importlib.invalidate_caches()
        if result.returncode != 0 or importlib.util.find_spec("tabfm") is None:
            raise RuntimeError(
                f"pip install of '{TABFM_PIP_SPEC}' failed. Google TabFM is the "
                "core engine of this experiment and has no fallback."
            )
    else:
        print("[env] tabfm already importable; using the installed version "
...
def _probe_and_benchmark(log, n_features: int, planned_fits: int) -> dict:
    ...
    surfaces within the first minute.
    """

    import numpy as np
    import pandas as pd

    from tabfmnested.modeling import TabFMModel

    rng = np.random.default_rng(0)

    # 1) tiny correctness probe
    X_tiny = pd.DataFrame(rng.normal(size=(24, 5)), columns=[f"p{i}" for i in range(5)])
    y_tiny = np.array([0, 1] * 12)
    model = TabFMModel(seed=0).fit(X_tiny, y_tiny)
    scores = model.predict_score(X_tiny.iloc[:8])
    if scores.shape != (8,) or not np.isfinite(scores).all():
        raise RuntimeError(f"TabFM probe emitted invalid scores: {scores!r}")
    if not (scores >= 0).all() or not (scores <= 1).all():
        raise RuntimeError("TabFM probe scores are not probabilities")
    log(f"[probe] API ok | positive column via {model.positive_column_source_} | "
    ...
    return {
        ...
        "positive_column_source": model.positive_column_source_,
    }
...
def _resolve_data_root(namespace: dict, explicit: str | None) -> Path:
    ...
    for value in (explicit, namespace.get("DATA_ROOT"), os.environ.get("BGTF_DATA_ROOT")):
        if value:
...
def _resolve_output_dir(explicit: str | None, run_id: str) -> Path:
    ...
    if os.environ.get("BGTF_OUTPUT_DIR"):
        return Path(os.environ["BGTF_OUTPUT_DIR"]).expanduser().resolve() / run_id
...
def _aggregate_tracks(oof_scores, y, diag, n_bootstrap, seed):
    ...
    from tabfmnested.evaluation import (
...
def main(namespace: dict) -> None:
    ...
    try:
        from tabfmnested import config as C

        stub_requested = os.environ.get(C.WIRING_STUB_ENV, "") == "1"
        _ensure_tabfm(stub_requested, arguments.profile)

        import numpy as np
        import pandas as pd
        import sklearn

        from tabfmnested.data import assert_disjoint_splits, load_labels, subject_hash
        from tabfmnested.evaluation import paired_bootstrap_auc_diff, thresholded_metrics
        from tabfmnested.features import (
            assert_no_forbidden, build_split_features, drop_degenerate_columns,
            feature_fingerprint,
        )
        from tabfmnested.modeling import (
            activate_wiring_stub, load_shared_model, shared_model_info,
        )
        from tabfmnested.nested_cv import run_repeated_nested_cv, select_and_fit_deployment
        ...
        # ------------------------------------------ model load + probe -------
        if stub_requested:
            engine_info = activate_wiring_stub()
            log("!! WIRING STUB ACTIVE (BGTF_WIRING_STUB=1): engine is NOT Google "
                "TabFM; this run checks plumbing only !!")
        else:
            log("[env] loading Google TabFM checkpoint (downloads from Hugging "
                "Face on first use) ...")
            engine_info = load_shared_model()
            log(f"[env] loaded: {engine_info}")
        ...
        run_config = {
            ...
            "google_technology": {
                "engine": "TabFM (Google Research tabular foundation model)",
                "role": (
                    "the model under test: sole learner of tabfm_mmse, the TabFM "
                    "half of blend_tabfm_lr, and the tabfm_fusion audit arm; "
                    "in-context learning, no dataset-specific training; "
                    "no fallback permitted"
                ),
                "pin": {"tag": C.TABFM_PIN_TAG, "commit": C.TABFM_PIN_COMMIT,
                        "pip_spec": C.TABFM_PIP_SPEC},
                "runtime": engine_info,
                "probe": probe,
                "weights_license": "tabfm-non-commercial-v1.0 (research use)",
            },
            "environment": {
        ...
        log(f"output          : {output}")
        log(f"seed            : {seed} (fold-parity with Binary_Google_CircadianNested)")
        ...
        log(f"google engine   : {engine_info}")
        log("=" * 78)
        ...
        contrast_pairs = [
            ("nested", "lr_mmse_c001"),
            ("tabfm_mmse", "lr_mmse_c001"),
            ("blend_tabfm_lr", "lr_mmse_c001"),
            ("tabfm_fusion", "tabfm_mmse"),
        ]
        ...
        # Cross-run anchor reproduction check (same seed & fold construction as
        # Binary_Google_CircadianNested 20260813_060156_utc).
        anchor_reproduction = None
        if "lr_mmse_c001" in tracks and profile.name in ("default", "max"):
            reference = C.BENCHMARK["circadian_nested_run"]
            observed = tracks["lr_mmse_c001"]["repeat_roc_auc_mean"]
            deviation = observed - reference["anchor_lr_mmse_c001_repeat_mean"]
            anchor_reproduction = {
                "reference_run": reference["source"],
                "reference_repeat_mean": reference["anchor_lr_mmse_c001_repeat_mean"],
                "observed_repeat_mean": float(observed),
                "deviation": float(deviation),
                "consistent_within_0p02": bool(abs(deviation) <= 0.02),
                "note": (
                    "large deviation flags an environment change, not a model "
                    "difference; treat the run as suspect if False"
                ),
            }
            log(f"[check] anchor reproduction: observed {observed:.4f} vs "
                f"reference 0.7617 (deviation {deviation:+.4f})")
        ...
        from tabfmnested.features import select_view
        ...
        from tabfmnested.evaluation import cn_vs_mci_auc, score_metrics
        validation_report = {
            "disclaimer": (
        ...
        # ---------------------------------------------------- final report ---
        stub_note = (
            "WIRING STUB RUN - engine was NOT Google TabFM; plumbing check only"
            if stub_requested else None
        )
        final_report = {
            "engine": shared_model_info(),
            "config": run_config,
            ...
            "anchor_reproduction_check": anchor_reproduction,
            "deployment": {
