"""End-to-end Google-YDF experiment: EDA, nested CV, freeze, evaluation."""
...
from .learners import YDF_AVAILABLE, YDFLearner

EXPERIMENT_NAME = "Binary_Google_YDF_Ensemble"
...
def _factories(data: SubjectData, config: RunConfig):
    return [
        ("ydf_gbt", lambda: YDFLearner(data, "gbt", seed=config.seed)),
        ("ydf_rf", lambda: YDFLearner(data, "rf", seed=config.seed)),
    ]
...
def run_experiment(config: RunConfig) -> dict:
    ...
    nested_report = {
        ...
        "google_model": "Yggdrasil Decision Forests (GBT + Random Forest)",
        "ydf_engine_used": bool(YDF_AVAILABLE),
        "engine_note": "ydf" if YDF_AVAILABLE else "sklearn HistGradientBoosting fallback (ydf not importable)",
        "cognitive_test_used": True,
...
def _freeze_and_evaluate(config, train, final_weights, thr_balanced, thr_accuracy, output):
    ...
    for name in final_weights:
        kind = "gbt" if name.endswith("gbt") else "rf"
        learner = YDFLearner(train, kind, seed=config.seed).fit(all_idx)
