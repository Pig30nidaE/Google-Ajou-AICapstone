...
def main():
    ...
    with open(report_md_path, "w", encoding="utf-8") as f:
        ...
        f.write("\n\n## 3. 핵심 결론 및 시사점\n\n")
        f.write("1. **33명 결측 복원 효과**: 0값 결함(정상군 26명 편중)을 해소하여 아티팩트에 의한 왜곡을 제거하고 순수 웨어러블 생체 리듬의 진짜 일반화 성능(AUC 0.6885)을 도출했습니다.\n")
        f.write("2. **SMOTE 제거의 우수성**: 가상 데이터를 생성하지 않는 Without SMOTE 조건이 거의 모든 모델에서 With SMOTE 대비 일관되게 더 높은 AUC를 기록했습니다.\n")
        f.write("3. **단일 XGBoost vs 앙상블**: 6개 모델을 결합한 앙상블(0.6818) 대비 단일 XGBoost(0.6885)가 더 우수한 성능을 보여, 복잡성을 낮춘 단일 모델의 실용적 가치를 확인했습니다.\n")
        f.write("4. **SHAP 변동성 신호 검증**: 장기 일간 변동성(`_std`) 피처들이 전체 중요도의 55.65%를 차지하여, 수면 뒤척임 및 활동 리듬의 불안정성이 인지저하의 핵심 바이오마커임을 입증했습니다.\n")
