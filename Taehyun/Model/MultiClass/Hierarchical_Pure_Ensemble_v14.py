...
def run_v14():
    ...
    # Generate 100 seeds
    seeds = np.random.RandomState(RANDOM_STATE).randint(0, 10000, size=SEED_COUNT)
    ...
    print(f"\n[최종 평가] V14 순수 100x 시드 앙상블 평가 시작 (Threshold 조정 없음)...")
    for fold, (tr_idx, te_idx) in enumerate(skf.split(df, df[TARGET_COL])):
        ...
        # Select top features inside fold using RF
    ...
    print("\n" + "="*80)
    print("FINAL RESULTS (V14: Pure 100x Seed Ensemble)")
    print("="*80)
    ...
    plt.title("V14 Pure 100x Seed Ensemble", fontsize=14, pad=10)
    plt.xlabel('Predicted Label', fontsize=12)
    plt.ylabel('True Label', fontsize=12)
    plt.tight_layout()
    plt.savefig(PLOT_DIR / "confusion_matrix_v14.png", dpi=150)
