...
def perform_forward_selection(all_data, all_feats, target_col):
    ...
    print("\n[단계 1] Feature Importance를 통해 피처 중요도 순위를 계산합니다 (SMOTE 적용)...")
    base_model = LGBMClassifier(objective="multiclass", num_class=3, random_state=RANDOM_STATE, n_jobs=1, class_weight='balanced', verbose=-1)
    ...
    for k in range(1, total_feats + 1):
        ...
        for train_idx, val_idx in cv.split(X_subset, y_train):
            ...
            prob = eval_model.predict_proba(X_va)
            fold_scores.append(roc_auc_score(y_va, prob, multi_class='ovr'))
...
def evaluate_model(model, X, y):
    ...
    return acc, prec, rec, f1, auc, confusion_matrix(y, pred)
...
def run_final_grid_search(all_data, selected_feats, target_col):
    ...
    for model_name, param_list in models_to_run:
        ...
        for params in param_list:
            ...
            for train_idx, valid_idx in cv_splitter.split(X_all, y_all):
                ...
                if model_name == "LightGBM":
                    model = LGBMClassifier(objective="multiclass", num_class=3, num_leaves=params[0], learning_rate=params[1], n_estimators=params[2], min_child_samples=params[3], max_depth=params[4], class_weight=params[5], random_state=42, n_jobs=1, verbose=-1)
                    model.fit(X_tr_res, y_tr_res, eval_set=[(X_va, y_va)], eval_metric='multi_logloss', callbacks=[lgb.early_stopping(50, verbose=False)])
                ...
                _, _, _, _, auc, _ = evaluate_model(model, X_va, y_va)
        ...
        for train_idx, test_idx in cv_splitter.split(X_all, y_all):
            ...
            if model_name == "LightGBM":
                final_model = LGBMClassifier(objective="multiclass", num_class=3, num_leaves=best_params[0], learning_rate=best_params[1], n_estimators=best_params[2], min_child_samples=best_params[3], max_depth=best_params[4], class_weight=best_params[5], random_state=42, n_jobs=1, verbose=-1)
            elif model_name == "RandomForest":
            ...
            acc, prec, rec, f1, auc, cm = evaluate_model(final_model, X_te, y_te)
...
plt.tight_layout()
plt.savefig(r"C:\Users\윤태현\.gemini\antigravity-ide\brain\d881410c-9019-4a0b-b882-dc261a2c2238\eda_plots\forward_selection_v3.png", dpi=150)
plt.show() 
#plt.close()
...
plt.tight_layout()
plt.savefig(r"C:\Users\윤태현\.gemini\antigravity-ide\brain\d881410c-9019-4a0b-b882-dc261a2c2238\eda_plots\confusion_matrix_v3.png", dpi=150)
plt.show() 
#plt.close()
