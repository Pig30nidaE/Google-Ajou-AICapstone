...
def perform_shap_forward_selection(df, features):
    ...
    for k in range(1, len(top_k_features) + 1):
        ...
        for train_idx, val_idx in skf.split(X_sub, y):
            ...
            eval_model.fit(X_tr_res, y_tr_res, eval_set=[(X_va, y_va)], callbacks=[lgb.early_stopping(30, verbose=False), lgb.log_evaluation(0)])
...
def optimize_and_run_v29(df, features):
    ...
    def objective_lgb(trial):
        ...
        for tr_i, va_i in skf.split(X, y):
            ...
            m.fit(X_tr_res, y_tr_res, eval_set=[(X_va, y_va)], callbacks=[lgb.early_stopping(30, verbose=False), lgb.log_evaluation(0)])
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
        ...
        m_lgb.fit(X_tr_res, y_tr_res, eval_set=[(X_te, y_te)], callbacks=[lgb.early_stopping(50, verbose=False), lgb.log_evaluation(0)])
...
def plot_v29_results(history_scores, optimal_k, results):
    ...
    plt.tight_layout()
    plt.savefig(PLOT_DIR / "confusion_matrix_v29_binary.png", dpi=150)
    plt.show()
