...
def numeric_sequence_stats(seq: np.ndarray, prefix: str, remove_zero: bool = False) -> dict:
    ...
    return {f"{prefix}_mean": float(np.mean(arr)), f"{prefix}_std": float(np.std(arr)), f"{prefix}_var": float(np.var(arr)), f"{prefix}_min": float(np.min(arr)), f"{prefix}_max": float(np.median(arr)), f"{prefix}_median": float(np.median(arr)), f"{prefix}_q25": float(q25), f"{prefix}_q75": float(q75), f"{prefix}_iqr": float(q75 - q25), f"{prefix}_valid_count": int(len(arr))}
...
def make_patient_table(daily_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "label" in daily_df.columns:
        labels = daily_df[["EMAIL", "original_label", "label"]].drop_duplicates()
        return df_combined.merge(labels, on="EMAIL", how="left")
    return df_combined

def process_raw_to_patient_level():
...
def run_scenario(X, y, scenario_name, leakage_allowed, use_nested_cv, n_repeats=5):
    print(f"\n=============================================")
    ...
    for repeat in range(n_repeats):
        ...
        if leakage_allowed:
            ...
            selector = RFECV(estimator=LogisticRegression(max_iter=500, random_state=42), step=1, cv=3, scoring='roc_auc')
            X_used = selector.fit_transform(X_sc, y)
        else:
            X_used = X
        ...
        for fold, (train_idx, test_idx) in enumerate(cv_split, 1):
            if leakage_allowed:
                ...
                selector = RFECV(estimator=LogisticRegression(max_iter=500, random_state=42), step=1, cv=3, scoring='roc_auc')
                X_tr = selector.fit_transform(X_tr_sc, y[train_idx])
                X_te = selector.transform(X_te_sc)
            ...
            if use_nested_cv:
                def objective(trial):
                    ...
                    for inner_tr, inner_val in inner_cv.split(X_tr, y_tr):
                        inner_model = LogisticRegression(C=C, class_weight='balanced', max_iter=500, random_state=42)
                ...
                study.optimize(objective, n_trials=10) # 속도를 위해 trials 수 제한
                model = LogisticRegression(C=study.best_params['C'], class_weight='balanced', max_iter=1000, random_state=42)
            else:
    ...
    print(f"\n[{scenario_name}] Final Statistical Results (5 Repeats x 10 Folds):")
    ...
    plt.title(f'KimPark LR - {scenario_name}')
    plt.xlabel('Predicted Label')
    plt.ylabel('True Label')
    plt.tight_layout()
    
    filename = f"KimPark_LR_{scenario_name.replace(' ', '_')}.png"
    plt.savefig(f'outputs/{filename}', dpi=300)
    plt.close()

def main():
    X, y, feature_names = process_raw_to_patient_level()
    ...
    for sc in scenarios:
        run_scenario(X, y, sc["name"], sc["leak"], sc["nested"], n_repeats=5)
