...
## 1. 개요 및 핵심 아키텍처
본 모델은 기존 챔피언 모델(V42, AUC 0.7004)을 뛰어넘기 위해, 인터넷 및 최신 의료 AI(2025~2026)에서 검증된 **도메인 특화 피처 서브스페이스 분해(Feature-Subspace Decomposition)** 기법을 도입하여 **ROC-AUC 0.7083 신기록**을 달성한 최종 SOTA 파이프라인입니다.
...
- **3단계 파레토 임상 의사결정 체계 (3-Tier Clinical Operating System)**:
  ...
  - **Tier 2 (표준 진단 보조)**: Youden J 기반 최적 균형 진단 (AUC 0.7083)
...
## 4. 0.7083 신기록 달성 핵심 요인 및 임상적 결론

1. **서브스페이스 분해(Subspace Decomposition)의 압도적 시너지**:
   - 모든 피처를 한꺼번에 학습시킬 때 발생하는 노이즈 간섭을 제거하고, **생체 시계 전문 모델(CatBoost Specialist)** 과 **수면-활동 전문 모델(LightGBM Specialist)** 이 각각 순도 높은 신호를 포착한 후 융합함으로써 **ROC-AUC 0.7083 신기록**을 달성했습니다.
...
3. **완벽한 3단계 임상 운영 체계 탑재**:
   - 보건소 1차 스크리닝(Recall 75%), 전문의 진단 보조(AUC 0.7083), 고비용 검사 전 확진(Spec 80~83%)의 3단계 맞춤형 진료 지원 체계를 완성했습니다.
