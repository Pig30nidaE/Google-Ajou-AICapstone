...
# Source notebook: Hyunsoo/previous/privious_TreeModel_preprocessing/build_rf_lgbm_binary_daily_colab.ipynb
...
# %% [markdown] cell 1
# # RF/LGBM 최종 이산형 데이터셋 생성 Colab 노트북
#
# `LGBM+RF/build_rf_lgbm_binary_daily.py`를 Colab 공유드라이브 환경에서 실행할 수 있게 변환한 노트북입니다.
#
# 이 스크립트는 RF/LGBM 학습용 최종 CSV인 `rf_lgbm_activity_discrete.csv`, `rf_lgbm_sleep_discrete.csv`, `rf_lgbm_discrete_dataset_guide.md`를 생성합니다. 기존 guide와 코드 검증 로직 기준으로 binary daily 이산형 모델 입력 데이터셋의 최종 생성 파일로 보아도 됩니다.
...
# User-defined DATA_ROOT
DATA_ROOT = Path('/content/drive/MyDrive/GoogleAI_contest/aihub_original_data')

# Assuming PROJECT_ROOT is the parent directory of the new DATA_ROOT
PROJECT_ROOT = DATA_ROOT.parent

# SCRIPT_DIR and OUTPUT_DIR are expected to be in a subdirectory of PROJECT_ROOT
# based on the original structure (PROJECT_ROOT / 'LGBM+RF')
SCRIPT_DIR = PROJECT_ROOT / 'LGBM+RF'
OUTPUT_DIR = PROJECT_ROOT / 'LGBM+RF'
...
# ## 원본 전처리 코드
#
# 노트북에서 `DATA_ROOT`, `SCRIPT_DIR`, `OUTPUT_DIR`를 먼저 지정할 수 있도록 경로 계산만 Colab 친화적으로 감쌌고, 나머지 전처리/검증 로직은 원본 `.py` 기준입니다.
...
# ## 실행
#
# 아래 셀을 실행하면 최종 RF/LGBM 이산형 CSV와 guide가 `OUTPUT_DIR`에 저장됩니다.

# %% cell 9
sys.argv = [
    'build_rf_lgbm_binary_daily_colab.ipynb',
